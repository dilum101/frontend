...
const TransactionPageContent = () => {
  ...
  useEffect(() => {
    if (role === 'user' && asset_addresses) {
      if (data?.hash === router.query.hash) {
        if (asset_addresses.includes(data?.from.hash as string)) {
  ...
  const tabs: Array<RoutedTab> = [
    ...
    config.features.suave.isEnabled && data?.wrapped ?
      {
        id: 'wrapped',
        title: 'Regular tx details',
        component: <TxDetailsWrapped data={ data.wrapped }/>,
      } :
      undefined,
    {
      id: 'token_transfers',
  ...
  const tags = (
    <EntityTags
      ...
      tagsBefore={ [
        data?.tx_tag ?
          { label: data.tx_tag, display_name: data.tx_tag } :
          undefined,
      ] }
    />
  );
  ...
  const backLink = React.useMemo(() => {
    const hasGoBackLink =
      appProps.referrer && appProps.referrer.includes('/txs');
  ...
  const titleSecondRow = (
    <TxSubHeading hash={ hash } hasTag={ Boolean(data?.tx_tag) }/>
  );

...
const AddressDetails = ({ addressQuery, scrollRef }: Props) => {
  ...
  const is422Error =
    addressQuery.isError &&
    "status" in addressQuery.error &&
    addressQuery.error.status === 422;

  if (addressQuery.isError && is422Error) {
    throw Error("Address fetch error", {
      cause: addressQuery.error as unknown as Error,
    });
  }
    ...
    >
      ...
      {data.is_contract && data.implementation_address && (
        ...
        >
          <AddressEntity
            address={{
              hash: data.implementation_address,
      ...
      {data.has_tokens && (
        ...
        >
          {addressQuery.data ? (
            <TokenSelect onClick={handleCounterItemClick} />
          ) : (
            <Box py="6px">0</Box>
          )}
        </DetailsInfoItem>
